...
class _ThongtincanhanState extends State<Thongtincanhan> {
  ...
  Future<void> _updateUserInfo(Map<String, dynamic> updatedInfo) async {
    try {
      await _userService.updateUserInfo(updatedInfo);
      _fetchUserInfo();  // Tải lại dữ liệu sau khi cập nhật thành công
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            InfoRow(icon: Icons.account_circle, label: 'Họ Tên:', value: _userInfo?['HoTen'] ?? 'Chưa cập nhật', onEdit: () => _showEditDialog('HoTen', _userInfo?['HoTen'] ?? '')),
                            InfoRow(icon: Icons.boy, label: 'Giới Tính:', value: _userInfo?['GioiTinh'] ?? 'Chưa cập nhật', onEdit: () => _showEditDialog('GioiTinh', _userInfo?['GioiTinh'] ?? '')),
                            InfoRow(icon: Icons.date_range, label: 'Ngày Sinh:', value: _userInfo?['NgaySinh'] ?? 'Chưa cập nhật', onEdit: () => _showEditDialog('NgaySinh', _userInfo?['NgaySinh'] ?? '')),
                            InfoRow(icon: Icons.location_on, label: 'Địa chỉ:', value: _userInfo?['DiaChi'] ?? 'Chưa cập nhật', onEdit: () => _showEditDialog('DiaChi', _userInfo?['DiaChi'] ?? '')),
                            InfoRow(icon: Icons.phone, label: 'Điện thoại:', value: _userInfo?['SoDienThoai'] ?? 'Chưa cập nhật', onEdit: () => _showEditDialog('SoDienThoai', _userInfo?['SoDienThoai'] ?? '')),
                            InfoRow(icon: Icons.mail, label: 'Email:', value: _userInfo?['Email'] ?? 'Chưa cập nhật', onEdit: () => _showEditDialog('Email', _userInfo?['Email'] ?? '')),
                          ],
                        ),
                      ),
                    ],
